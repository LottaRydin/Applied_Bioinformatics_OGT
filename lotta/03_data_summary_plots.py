...
metadata = pd.read_csv('temp_samples_combined.tsv', sep='\t') #specify metadata file
...
sns.violinplot(x="taxonomy", y="temp", data=violin_df, scale="count", inner="stick", ax=axs[0]) #If count, the width of the violins will be scaled by the number of observations in that bin. 
axs[0].tick_params(axis='x', rotation=90)
axs[0].set_title("Outliers not removed")
#fig = ax.get_figure()
#plt.show()
...
violin_df_f = []
for indx in rand_otu:
    otu_data = df.loc[indx, :]
    otu_temp = df.loc[indx, :].temp
    z_scores = zscore(otu_temp)
    abs_z_scores = np.abs(z_scores)
    filtered_entries = (abs_z_scores < z_cutoff) #.all(axis=1)
    new_df = otu_data[filtered_entries]

    violin_df_f.append(new_df)

violin_df_f = pd.concat(violin_df_f)
violin_df_f.reset_index(inplace=True)

#ax_f = sns.violinplot(x="taxonomy", y="temp", data=violin_df_f, scale="count", inner="stick") #If count, the width of the violins will be scaled by the number of observations in that bin. 
sns.violinplot(x="taxonomy", y="temp", data=violin_df_f, scale="count", inner="stick", ax=axs[1]) #If count, the width of the violins will be scaled by the number of observations in that bin. 
axs[1].tick_params(axis='x', rotation=90)
axs[1].set_title("Outliers removed")
# ax_f.tick_params(axis='x', rotation=90)
# ax_f.set_title("Outliers removed")
# fig_f = ax_f.get_figure()
plt.show()
plt.savefig('violin_plot_f.png')
...
def tempura_and_prediction(OTU_nr):
    ...
    # remove outliers
    otu_data = df.loc[OTU_nr, :]
    otu_temp = df.loc[OTU_nr, :].temp
    z_scores = zscore(otu_temp)
    abs_z_scores = np.abs(z_scores)
    filtered_entries = (abs_z_scores < z_cutoff) #.all(axis=1)
    new_df = otu_data[filtered_entries]
    ...
    # Plot
    plt.scatter(df.loc[OTU_nr, :].temp, df.loc[OTU_nr, :].rel_abundance, alpha=0.5, c='blue')
    plt.scatter(new_df.temp, new_df.rel_abundance, alpha=0.5, c='green')
    ...
    plt.savefig("../../plots_all/"+str(OTU_nr)+"_otu.png")
    plt.show()
...
cols = ['OTU_ID', 'Tmin', 'Topt', 'Tmax']
temps_pred_f = []
tempura_matches_f = list(set(df.index.unique("OTU_ID")) & set(tempura_otu_df.index.unique("OTU_id_5")))
print(tempura_matches_f)
for otu in df.index.unique("OTU_ID"):
    # remove outliers
    otu_data = df.loc[otu, :]
    otu_temp = df.loc[otu, :].temp
    z_scores = zscore(otu_temp)
    abs_z_scores = np.abs(z_scores)
    filtered_entries = (abs_z_scores < z_cutoff) #.all(axis=1)
...
temps_pred_f = pd.DataFrame(temps_pred_f, columns=cols)
del cols
del temps_est

temps_pred_f = temps_pred_f.set_index(["OTU_ID"]) #set OTU col as the index
temps_pred_f.sort_index(inplace=True) #sort dataframe based on increasing index values

temps_compare_f = temps_pred_f.loc[tempura_matches_f]
temps_compare_f = temps_compare_f.reindex(columns=['Tmin', 'Topt', 'Tmax', 'temp_Tmin', 'temp_Topt', 'temp_Tmax'])
temps_compare_f.loc[:,'temp_Tmin':'temp_Tmax'] = tempura_otu_df.loc[temps_compare_f.index,'Tmin':'Tmax'].values

def regression_plot(x, y, x_f, y_f, xlab, ylab, title):
...
regression_plot(temps_compare.temp_Topt, temps_compare.Topt, temps_compare_f.temp_Topt, temps_compare_f.Topt, "TEMPURA Topt", "predicted Topt", "Topt_prediction",)
regression_plot(temps_compare.temp_Tmin, temps_compare.Tmin, temps_compare_f.temp_Tmin, temps_compare_f.Tmin, "TEMPURA Tmin", "predicted Tmin", "Tmin_prediction")
regression_plot(temps_compare.temp_Tmax, temps_compare.Tmax, temps_compare_f.temp_Tmax, temps_compare_f.Tmax, "TEMPURA Tmax", "predicted Tmax", "Tmax_prediction")

regression_plot(tempura.Tmin, tempura.Tmax, tempura.Tmin, tempura.Tmax, "TEMPURA Tmin", "TEMPURA Tmax", "TEMPURA_tmax_tmin")
regression_plot(temps_pred.Tmin, temps_pred.Tmax, temps_pred_f.Tmin, temps_pred_f.Tmax,  "prediction Tmin", "prediction Tmax", "Prediction_tmax_tmin")

regression_plot(tempura.Tmin, tempura.Topt_ave, tempura.Tmin, tempura.Topt_ave, "TEMPURA Tmin", "TEMPURA Topt", "TEMPURA_topt_tmin")
regression_plot(temps_pred.Tmin, temps_pred.Topt, temps_pred_f.Tmin, temps_pred_f.Topt, "prediction Tmin", "prediction Topt", "Prediction_topt_tmin")
...
regression_plot(temps_compare.temp_Tmax-temps_compare.temp_Tmin,\
                ...
                temps_compare_f.temp_Tmax-temps_compare_f.temp_Tmin,\
                temps_compare_f.Tmax-temps_compare_f.Tmin, \
