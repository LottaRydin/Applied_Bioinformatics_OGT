...
def leave_1_out(df, min_read, min_samp, ratio, min_z, max_z, xy_transform = False):
    ...
    tempura_matches_max = list(set(df_filt.index.unique("OTU_ID")) & set(tempura_otu_df.index.unique("OTU_id_5")))
    cols = ['OTU_ID', 'Tmin', 'Topt', 'Tmax']
    temps_pred = []
    temps_pred_max = []
    ...
    for otu in tempura_matches_1:
        ...
        if len(max_df.temp) > 0 and len(min_df.temp) > 0:
            ...
            temps_pred_max.append(temps_est)
            #print(max_est)
        elif len(max_df.temp) > 0:
            max_est = max(max_df.temp)
            temps_est = [otu, None, None, max_est]
            temps_pred_max.append(temps_est)
            ...
            if otu in tempura_matches:
                tempura_matches.remove(otu)
                tempura_matches_max.remove(otu)
    ...
    #print(temps_pred)
    temps_pred_max = pd.DataFrame(temps_pred_max, columns=cols) 
    temps_pred_max = temps_pred_max.set_index(["OTU_ID"]) #set OTU col as the index
    temps_pred_max = temps_pred_max.reindex(columns=['Tmin', 'Topt', 'Tmax', 'TEMP_Tmin', 'TEMP_Topt', 'TEMP_Tmax'])
    temps_pred_max.loc[:,'TEMP_Tmin':'TEMP_Tmax'] = tempura_otu_df.loc[temps_pred_max.index,'Tmin':'Tmax'].values

    print(f"Z max cutoff: {max_z}, Z min cutoff: {min_z}")
    for type_T in ["min", "max", "opt"]:
        ...
        if type_T == "max":
            loop = tempura_matches_max
            pred_data = temps_pred_max
        else:
            loop = tempura_matches
            pred_data = temps_pred
        #print(pred_data)
        ...
        for otu in tempura_matches:
            #print(f'--------------OTU: {otu}-------------')
            #print(temps_pred)
            train_set = pred_data.iloc[lambda x: x.index != otu]
            ...
            # Values for test otu
            otu_TEMP = pred_data.iloc[lambda x: x.index == otu,col_dict[type_T][0]].values.reshape(-1, 1)
            otu_pred = ((pred_data.iloc[lambda x: x.index == otu,col_dict[type_T][1]]-b)/a).values.reshape(-1, 1)
            otu_mmo = pred_data.iloc[lambda x: x.index == otu,col_dict[type_T][1]].values.reshape(-1, 1)
            ...
            #print(otu_TEMP)
            #print(otu_pred)
            #print(otu_TEMP-otu_pred)

            error = float(np.abs(otu_TEMP-otu_pred, dtype=object))
            true_all.append(otu_TEMP[0])
            pred_all.append(otu_pred[0])
            e_all.append(error)

            plt.scatter(otu_TEMP, otu_pred )
            plt.annotate(round(error, 1), (otu_TEMP, otu_pred ))
        ...
        true_rmse = math.sqrt(mse)
        plt.figtext(0.28,0.73,"mean error: "+str("{:.3f}".format(mean(e_all)))+"\n"+\
            "rmse: "+str("{:.3f}".format(mean(true_rmse)))+"\n"+\
            "{}\u00b2".format("R")+": "+str("{:.3f}".format(r2))+"\n"+\
            #f"Abs diff: {abs}"+"\n"+\
            f"Z max cutoff: {max_z}, Z min cutoff: {min_z}", size = "small")
        ...
        #print(e_all)
        print(f'{type_T}: mean e: {mean(e_all)}, rmse: {true_rmse} R^2: {r2}')
        plt.hist(e_all, bins=40)
        plt.ylabel("Frequency")
        plt.xlabel("Error")
        plt.show()
    print(f'tempura matches used: {len(tempura_matches)}, max: {len(tempura_matches_max)}')

leave_1_out(df, p_min_read, p_min_samp, p_ratio, p_z_min, p_z_max, xy_transform=True) #opt
#leave_1_out(df, p_min_read, p_min_samp, p_ratio, p_z_max, p_z_max, xy_transform=True) #opt