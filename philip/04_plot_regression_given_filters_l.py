...
def regression_filter_plot(type_T, min_read, min_samp, diff_from_max, max_z, xy_transform = False):
            ...
            for otu in tempura_matches:
                optimum_est = (max(df_filt.loc[otu, :].temp)) - (diff_from_max)
            ...
            for otu in tempura_matches:
                ...
                if len(f_df.temp) > 0:
                    optimum_est = max(f_df.temp) - diff_from_max
...
regression_filter_plot("opt", p_min_read, p_min_samp, p_diff_from_max, p_z_cutoff, xy_transform=False) #opt
regression_filter_plot("opt", p_min_read, p_min_samp, p_diff_from_max, p_z_cutoff, xy_transform=True) #opt
#regression_filter_plot("min", p_min_read, p_min_samp, p_diff_from_max, p_z_cutoff,) #min
#regression_filter_plot("min", p_min_read, p_min_samp, p_diff_from_max, p_z_cutoff, xy_transform=True) #min
#regression_filter_plot("max", p_min_read, p_min_samp, p_diff_from_max, p_z_cutoff,) #max
#regression_filter_plot("max", p_min_read, p_min_samp, p_diff_from_max, p_z_cutoff, xy_transform=True) #max
