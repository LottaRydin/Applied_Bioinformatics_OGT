...
# Fin matches in tempura for training
...
for type_T in ["opt", "min", "max"]:
    ...
    if type_T == "max":
        pred_data_train = temps_pred_max
    else:
        pred_data_train = temps_pred_train
    ...
    X = pred_data_train.iloc[:,col_dict[type_T][0]].values.reshape(-1, 1)
    Y = pred_data_train.iloc[:,col_dict[type_T][1]].values.reshape(-1, 1)
