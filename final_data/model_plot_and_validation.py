...
def regression_filter_plot_original(type_T, min_read, min_samp, ratio, min_z, max_z, xy_transform = False):
    ...
    if type_T == "min":
        otu_plot = list(set(min_df.index.unique("OTU_ID")) & set(tempura_matches))
        max_df = min_df
    elif type_T == "max":
        otu_plot = list(set(max_df.index.unique("OTU_ID")) & set(tempura_matches))
        min_df = max_df
    else:
...
#                                       set(tempura_otu_df.index.unique("OTU_id_5")))),"OTUs plotted",  "for", type_T, "regression")
    print("OTUs plotted", len(temps_pred), "for", type_T)
