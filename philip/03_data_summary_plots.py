...
#train_set, test_set = split_train_test(temps_compare, 0.2)
#print('train: '+ str(len(train_set)))
#print('test: '+ str(len(test_set)))

#def regression_plot(x_train, y_train, x_test, y_test, xlab, ylab, title):
...
#regression_plot(train_set.temp_Topt, train_set.Topt, test_set.temp_Topt, test_set.Topt, "TEMPURA Topt", "predicted Topt", "Topt_prediction",)
...
#X_train, X_test, y_train, y_test = train_test_split(x, y, test_size=0.5, random_state=137)
...
cvd = LeaveOneOut
model = lr
scores = cross_val_score(model, x, y, scoring='neg_mean_squared_error', cv=cvd, n_jobs=-1)
print("LoO RMSE:")
print(mean(np.sqrt(-scores)))                     
                     
ori_pred= lr.predict(x)
ori_mse= mean_squared_error(y, ori_pred)
